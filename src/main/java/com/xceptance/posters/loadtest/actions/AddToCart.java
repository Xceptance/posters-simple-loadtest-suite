 ...
 * This action does not result in a page load but consists of a sequence of
 * three AJAX calls. JavaScript is disabled due to performance reasons. So
 * assembling the request parameters, make the call and evaluating the response
 * content makes this kind of actions a bit more complex.
 */
public class AddToCart extends AbstractHtmlPageAction {
	...
	public void preValidate() throws Exception {
		...
		finish = HtmlPageUtils
				.findHtmlElementsAndPickOne(addToCartForm, "id('product-detail-form-style-selection')//label")
				.getTextContent().trim();
		...
		final HtmlElement option = HtmlPageUtils.findHtmlElementsAndPickOne(page,
				"id('product-detail-form-size-selection')/option");
		...
		productId = HtmlPageUtils.findSingleHtmlElementByXPath(page, "id('product-detail-form-product-id')")
				.getTextContent();
		...
		Assert.assertTrue("AddToCart button is not present.",
	...
	protected void execute() throws Exception {
		...
		final WebResponse updatePriceResponse = AjaxUtils.callPost(page, contextPath + "/updatePrice",
				updatePriceParams);
		...
		// Note: This is not necessary since we just want to simulate realistic traffic
		// for the server and normally do
		...
		final List<HtmlElement> oldCartItems = page
				.getByXPath("id('miniCartMenu')//li[contains(@class, 'miniCartItem')]");
		...
		// Perform the AJAX call and return the result.
		final WebResponse addToCartResponse = AjaxUtils.callGet(page, contextPath + "/addToCartSlider",
				addToCartParams);

		// Get JSON object from response.
		final JSONObject addToCartJsonResponse = new JSONObject(addToCartResponse.getContentAsString());

		// (4) Check that the right item was added to cart and that the product's
		// quantity has been increased by 1 if it
		// was already in cart.
		...
		Assert.assertEquals("The addToCart call returned the wrong price", newPrice.substring(1),
				product.get("productUnitPrice"));
		Assert.assertEquals("The addToCart call returned the wrong product ID", Integer.parseInt(productId),
				product.getInt("productId"));

		// Get the added product's properties like ID, finish, weight, and height. This
		...
		// Now search the initial cart for the added product by comparing the product
		// properties.
		...
		// It's expected that the item quantity for that special product has been
		// increased by 1.
		Assert.assertEquals("The addToCart call returned the wrong count", cartItemQuantity + 1,
				product.getInt("productCount"));
	...
	protected void postValidate() throws Exception {
		...
		// and all the JSON responses have been validated already in the execute()
		// method
