...
public class Homepage extends AbstractHtmlPageAction {
	...
	/**
	 * Constructor. This will be called from a test case and while doing so the
	 * homepage's URL will be passed so that the page can eventually be loaded.
	 * 
	 * @param urlAsString the URL to fetch the data from
	 */
	...
	/**
	 * Execute the request. Once pre-execution conditions have been meet, the
	 * execute method can be called to load the page, in this case the homepage will
	 * be requested.
	 */
	@Override
	...
	/**
	 * Validate the correctness of the result. Once the homepage has been loaded, we
	 * can ensure that certain key elements are present in our previous request's
	 * responses. For example, here we are validating that the proper response code
	 * was sent, the length of the page is correct, an end tag is present, there is
	 * a head line on the page. This is all being done with the help of validators.
	 * Validators are used when we need to check the same thing after several
	 * different actions.
	 */
	@Override
