...
public class NavBarValidator {
	...
	public void validate(final HtmlPage page) throws Exception {
		...
		// size of the list
		final List<HtmlElement> topCategories = HtmlPageUtils.findHtmlElements(page,
				"id('header-categories')/li[@class='nav-item dropdown']");
		Assert.assertTrue("There are less then two top categories in the top nav.", topCategories.size() >= 2);
		...
		for (final Iterator<HtmlElement> iterator = topCategories.iterator(); iterator.hasNext();) {
			...
			Assert.assertTrue("Top category is not followed by a level-1 category.",
					!page.querySelectorAll("#header-categories .dropdown-menu").isEmpty());
		}
